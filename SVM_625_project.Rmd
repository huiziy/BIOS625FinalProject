...
classifier_without = svm(formula = anemia ~ .,
                 data = training_set,
...
start.time = proc.time()
classifier_with = svm(formula = anemia ~ .,
                 data = training_set,
                 type = 'C-classification',
                 kernel = 'linear')
stop.time = proc.time()
...
```{r}
y_pred = predict(classifier_with, newdata = temp_data[c("age", "edu", "marriage", "job", "wealth", "head", "media", "toilet", "water", "term_preg", "housesize", "facility_dist", "contraceptive", "pregn_now", "breastfeeding", "residence", "poverty_rate", "literacy_rate")])
